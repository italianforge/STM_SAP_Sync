...
Le notifiche schedulate vengono accodate su STM_Scheduler; l'invio SMTP
è gestito dal servizio STM_Email_Service.
"""
import os
import time
import signal
...
def _scheduler_auth_headers() -> dict:
    """Auth headers for machine-to-machine calls to STM_Scheduler."""
...
def _trigger_scheduled_notifications(logger, now: 'datetime') -> None:
    """
    Check if rfq_digest, understock_report or intervention_reminders should fire for the current hour.
    Calls the STM_Scheduler enqueue endpoints which handle deduplication.
    """
    import requests

    base = _get_scheduler_base_url()
    headers = _scheduler_auth_headers()
    ...
    try:
        ...
        with engine.connect() as conn:
            rows = conn.execute(sa_text(
                ...
                "'intervention_reminder_enabled','intervention_reminder_hour')"
            )).fetchall()
        engine.dispose()
    ...
    if str(cfg_map.get('rfq_digest_enabled', 'false')).lower() == 'true':
        ...
        if current_hour == digest_hour:
            try:
                resp = requests.post(f"{base}/api/v1/resources/notifications/enqueue/rfq-digest", headers=headers, timeout=30)
    ...
    if str(cfg_map.get('understock_report_enabled', 'false')).lower() == 'true':
        ...
        if current_hour == report_hour:
            try:
                resp = requests.post(f"{base}/api/v1/resources/notifications/enqueue/understock-report", headers=headers, timeout=30)
                if resp.status_code == 409:
    ...
    if str(cfg_map.get('intervention_reminder_enabled', 'false')).lower() == 'true':
        try:
            reminder_hour = int(cfg_map.get('intervention_reminder_hour', '7'))
        except (ValueError, TypeError):
            reminder_hour = 8
        if current_hour == reminder_hour:
            try:
                resp = requests.post(
                    f"{base}/api/v1/resources/notifications/enqueue/intervention-reminders",
                    headers=headers,
                    timeout=30,
                )
                if resp.status_code == 200:
                    data = resp.json()
                    logger.info(
                        f"intervention_reminders trigger: enqueued={data.get('enqueued')} "
                        f"skipped={data.get('skipped')} errors={data.get('errors')}"
                    )
                else:
                    logger.warning(
                        f"intervention_reminders trigger HTTP {resp.status_code}: {resp.text[:200]}"
                    )
            except Exception as e:
                logger.warning(f"intervention_reminders trigger error: {e}")
